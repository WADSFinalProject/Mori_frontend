...
const XYZShipDetails = () => {
  ...
  const getEstimatedArrivalTextColor = (status) => {
    switch (status) {
      case "PKG_Delivered":
        return "#FFFFFF"; // White for Need Pickup
      case "PKG_Delivering":
        return "#000000"; // Black for Shipping
      case "Missing":
        return "#000000"; // Black for Missing
      case "XYZ_PickingUp":
        return "#FFFFFF"; // White for Picking Up
      case "XYZ_Completed":
  ...
  const getStatusTextColor = (status) => {
    switch (status) {
      case "PKG_Delivered":
        return "#B8F3D3"; // Light green for Need Pickup
      case "PKG_Delivering":
        return "#217045"; // Green for Shipping
      case "Missing":
        return "#852222"; // Dark red for Missing
      case "XYZ_PickingUp":
        return "#B8F3D3"; // Light green for Picking Up
      case "XYZ_Completed":
  ...
  const getExpeditionTextColor = (status) => {
    switch (status) {
      case "PKG_Delivered":
        return "#B8D4C5"; // Light green for Need Pickup
      case "PKG_Delivering":
        return "#00000066"; // Grey for Shipping
      case "Missing":
        return "#5E4949"; // Dark grey for Missing
      case "XYZ_PickingUp":
        return "#B8D4C5"; // Light green for Picking Up
      case "XYZ_Completed":
  ...
  return (
    <div className="max-w-[425px] mx-auto h-screen flex flex-col items-start justify-start bg-white">
      <header className="w-full flex flex-row items-center justify-between gap-12 px-6 bg-white py-4">
        <Link to={"/xyz/m/shippinginformation"}>
          <svg
            width="20"
            height="17"
            viewBox="0 0 20 17"
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white py-3 px-5 border-t-2 border-gray-200">
        <button
          className={`w-full rounded-lg px-5 py-2 font-medium text-sm ${
            isConfirmArrivalDisabled
              ? "bg-[#D9D9D9] text-[#00000033] cursor-not-allowed"
              : "bg-[#5C612C] text-[#F2F9A9] hover:bg-[#434924]"
          }`}
          onClick={handleConfirmArrival}
          disabled={isConfirmArrivalDisabled}
        >
          Confirm Arrival
        </button>
      </div>
